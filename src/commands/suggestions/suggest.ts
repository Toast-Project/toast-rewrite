...
export default class extends Command {
    ...
    public async run(client: ToastClient, interaction: CommandInteraction) {
        ...
        const suggestionMessage = await suggestionChannel.send({ embeds: [suggestionEmbed] })
            .catch(e => {
                return interaction.reply({
                    content: `<:no:811763209237037058> The following error occurred while attempting to send this suggestion:\n\`\`\`${e}\`\`\``,
                    ephemeral: true
                });
            });
        ...
        await client.db.suggestions.insert(newSuggestion)
            .catch(e => {
                return interaction.reply({
                    content: `<:no:811763209237037058> The following error occurred while attempting to save this suggestion:\n\`\`\`${e}\`\`\``,
                    ephemeral: true
                });
            });
