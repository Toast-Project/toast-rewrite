...
export default class extends Command {
    ...
    public async run(client: ToastClient, interaction: CommandInteraction) {
        ...
        await notify(interaction.guild, user, "ban", Date.now(), null, <string>reason || "No reason provided.")
            .catch(e => e);

        await guild.members.ban(user, { reason: reason?.toString() || "No reason provided" })
            .catch(e => {
                return interaction.reply({
        ...
        return interaction.reply(`<:check:811763193453477889> \`${user.tag}\` has been banned${reason ? ` for \`${reason}\`.` : "."}`);
    }
}