...
export default class extends Command {
    ...
    public async run(client: ToastClient, interaction: CommandInteraction) {
        ...
        switch (subCommand) {
                ...
                await client.db.guilds.setLogChannel(interaction.guild.id, null)
                    .catch(e => {
                        return interaction.reply({
                            content: `<:no:811763209237037058> The following error occurred while attempting to set the log channel:\n\`\`\`${e}\`\`\``,
                            ephemeral: true
                        });
                    });
            ...
            case "set":
                await client.db.guilds.setLogChannel(interaction.guild.id, channel.id)
                    .catch(e => {
                        return interaction.reply({
                            content: `<:no:811763209237037058> The following error occurred while attempting to set the log channel:\n\`\`\`${e}\`\`\``,
                            ephemeral: true
                ...
                return interaction.reply(`<:check:811763193453477889> The log channel for the server has successfully been set to ${channel}.`);
        }
    }
}