    ...
    public async run(client: ToastClient, interaction: CommandInteraction) {
        ...
        switch (subCommand) {
            case "view":
                ...
                if (!viewCmd) return interaction.reply(`<:no:811763209237037058> The command provided does not exist.`, { ephemeral: true });
            ...
            case "set":
                ...
                if (level > 4 || level < 0) return interaction.reply(`<:no:811763209237037058> You must provide a number between 0 and 4.\n\nPermission Levels:\n0 = No Permissions\n1 = Moderator Role\n2 = Administrator Role\n3 = Administrator Permissions\n4 = Server Owner`, { ephemeral: true });
                ...
                await client.db.guilds.setPermissionLevel(interaction.guild.id, cmd.help.name, <number>level)
                    .catch(e => {
                        return interaction.reply(`<:no:811763209237037058> The following error occurred while attempting to set the permission level:\n\`\`\`${e}\`\`\``, { ephemeral: true });
                    });
