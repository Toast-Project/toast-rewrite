...
export default class extends SlashCommand {
    ...
    public async run(client: ToastClient, interaction: CommandInteraction) {
        ...
        const resolvedUser = await interaction.guild.members.fetch(<Snowflake>user)
            .catch(e => {
        ...
        if (lastRob + timeout > Date.now()) {
            ...
            return interaction.reply(`You must wait ${time} before attempting to rob someone.`, { ephemeral: true });
        }
        ...
        if (!memberData?.balance || memberData.balance < 50) {
            return interaction.reply("The user you are attempting to rob is too poor.",{ ephemeral: true });
        }
