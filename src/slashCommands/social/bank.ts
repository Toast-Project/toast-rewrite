...
export default class extends SlashCommand {
    ...
    public async run(client: ToastClient, interaction: CommandInteraction) {
        ...
        let amount = <number>interaction.options[0].options?.[0].value || <number>interaction.options[1].options?.[0].value;
        ...
        switch (subCommand) {
            case "deposit":
                if (amount < 1 || balance < 1) return interaction.reply(`You must provide an amount larger than ${symbol}1, and you must have at least ${symbol}1 in your balance.`, { ephemeral: true });
            ...
            case "withdraw":
                if (amount < 1 || bank < 1) return interaction.reply(`You must provide an amount larger than ${symbol}1, and you must have at least ${symbol}1 in your bank account.`, { ephemeral: true });
